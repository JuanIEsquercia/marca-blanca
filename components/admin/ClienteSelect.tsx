...
export default function ClienteSelect({ compradores, value, onChange }: Props) {
  ...
  return (
    <div className="space-y-2">
      {value.compradorId ? (
        <div className="flex items-center justify-between gap-2 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 rounded-lg px-3 py-2">
          <p className="text-xs text-indigo-700 dark:text-indigo-300">Cliente existente: <strong>{value.nombre}</strong></p>
          <button type="button" onClick={cambiar} className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-200 font-medium shrink-0">Cambiar</button>
        </div>
      ) : (
            ...
            <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
              {resultados.map(c => (
                <button key={c.id} type="button" onMouseDown={() => elegir(c)}
                  ...
                  <p className="font-medium text-slate-800 dark:text-slate-200">{c.nombre_completo}</p>
                  {c.dni_cuit && <p className="text-xs text-slate-400 dark:text-slate-500">{c.dni_cuit}</p>}
                </button>
              ))}
            </div>
          )}
      ...
      {colision && (
        <div className="flex items-center justify-between gap-2 bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
          <p className="text-xs text-amber-700 dark:text-amber-300">Ya existe un cliente con este CUIT: <strong>{colision.nombre_completo}</strong></p>
          <button type="button" onClick={() => elegir(colision)} className="text-xs text-amber-800 dark:text-amber-200 font-medium hover:underline shrink-0">Usar sus datos</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">CUIT/DNI</label>
