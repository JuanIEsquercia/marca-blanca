  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <p className="text-slate-500 dark:text-slate-400 text-sm">{tipologias.length} tipología(s) cargadas</p>
        {!readOnly && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {tipologias.map(t => (
          <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs transition-all">
            ...
            <div className="relative h-40 bg-slate-100 dark:bg-slate-800 overflow-hidden">
              {t.imagen_portada ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <p className="text-xs text-slate-400 dark:text-slate-500">Sin imagen de portada</p>
                </div>
              )}
              {/* Overlay botón 360 */}
              {t.url_recorrido_360 && (
                <button
                  ...
                  className="absolute bottom-2 right-2 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg
                             bg-black/60 text-white text-xs hover:bg-black/80 transition-colors backdrop-blur-xs">
            ...
            <div className="p-4">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-semibold text-slate-900 dark:text-white">{t.nombre}</h3>
                <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200/50 dark:border-indigo-800/50 px-2 py-0.5 rounded-full">
                  {t.m2_totales} m² tot.
                </span>
              </div>
              ...
              {t.descripcion && (
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 line-clamp-2">{t.descripcion}</p>
              )}
              {!readOnly && (
                <div className="flex gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                  <button onClick={() => openEdit(t)}
                    className="flex-1 text-xs text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors">
                    Editar
                  </button>
                  <span className="text-slate-200 dark:text-slate-800">|</span>
                  <button onClick={() => handleDelete(t.id, t.nombre)}
                    className="flex-1 text-xs text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 font-medium transition-colors">
                    Eliminar
                  </button>
                </div>
              )}
      ...
      {tipologias.length === 0 && (
        <div className="text-center py-16 text-slate-400 dark:text-slate-500">
          ...
          {!readOnly && (
            <button onClick={openNew} className="text-indigo-500 dark:text-indigo-400 text-sm hover:text-indigo-700 dark:hover:text-indigo-300">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">m² propios *</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">m² comunes</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                  URL recorrido 360°
                  <span className="ml-1 font-normal text-slate-400 dark:text-slate-500">(Kuula, Matterport, Panoee...)</span>
                </label>
              ...
              {/* Imagen de portada */}
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Imagen de portada</label>
                {form.imagen_portada ? (
                  <div className="relative mb-2 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <button type="button"
                    ...
                    className="w-full h-24 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl text-slate-400 dark:text-slate-500
                               hover:border-indigo-400 dark:hover:border-indigo-500 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors text-sm
