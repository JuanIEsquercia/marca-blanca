...
const ESTADO_INFO: Record<EstadoEquipo, { label: string; color: string }> = {
  disponible:    { label: 'Disponible',    color: 'bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400' },
  asignado:      { label: 'Asignado',      color: 'bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-400' },
  mantenimiento: { label: 'Mantenimiento', color: 'bg-amber-100 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400' },
  baja:          { label: 'Baja',          color: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' },
}
    ...
    <div className="space-y-5">
      <div className="flex flex-col md:flex-row gap-3 justify-between">
        <input
          ...
          className="w-full md:w-64 px-3 py-2 border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 rounded-lg text-sm
                     focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <div className="flex rounded-lg border border-slate-300 dark:border-slate-800 overflow-x-auto max-w-full text-sm bg-white dark:bg-slate-900 w-fit no-scrollbar">
              ...
              <div key={equipo.id}>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-slate-900 dark:text-white">{equipo.nombre}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                    ...
                    {(equipo.estado === 'disponible' || equipo.estado === 'asignado') && (
                      <button onClick={() => liberar(equipo, 'mantenimiento')}
                        className="text-xs px-2.5 py-1.5 border border-amber-200 dark:border-amber-800/60 text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 rounded-lg transition-colors">
                        Mantenimiento
                      </button>
                    )}
                    {(equipo.estado === 'mantenimiento' || equipo.estado === 'baja') && (
                      <button onClick={() => liberar(equipo, 'disponible')}
                        className="text-xs px-2.5 py-1.5 border border-emerald-200 dark:border-emerald-800/60 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 rounded-lg transition-colors">
                        Reactivar
                      </button>
                    )}
                    {equipo.estado !== 'baja' && (
                      <button onClick={() => liberar(equipo, 'baja')}
                        className="text-xs px-2.5 py-1.5 border border-red-200 dark:border-red-800/60 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
                        Dar de baja
                      </button>
                    )}
                ...
                {isExpanded && (
                  <div className="bg-slate-50 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 px-5 py-4 space-y-3">
                    {equipo.nro_serie && <p className="text-xs text-slate-500 dark:text-slate-400">N° de serie: {equipo.nro_serie}</p>}
                    {equipo.notas && <p className="text-xs text-slate-500 dark:text-slate-400 italic">{equipo.notas}</p>}

                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Trazabilidad — historial de asignaciones</p>
                    {historial.length === 0 ? (
                      <p className="text-xs text-slate-400 dark:text-slate-500">Este equipo todavía no fue asignado a ningún proyecto.</p>
                    ) : (
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl divide-y divide-slate-100 dark:divide-slate-800">
                        {historial.map(a => (
                          <div key={a.id} className="px-4 py-2.5 grid grid-cols-[1fr_auto] items-center gap-3 text-sm">
                            ...
                            {a.fecha_hasta === null && (
                              <span className="text-xs px-2 py-0.5 rounded font-medium bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 shrink-0">Vigente</span>
                            )}
                          </div>
                        ))}
                      </div>
      ...
      {/* Modal: nuevo equipo */}
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs" onClick={() => setShowForm(false)}>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Nuevo equipo</h2>
            ...
            <form onSubmit={handleCrearSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">N° de serie</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Marca</label>
              ...
              {error && <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">{error}</p>}
      ...
      {asignarTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs" onClick={() => setAsignarTarget(null)}>
            ...
            <form onSubmit={handleAsignarSubmit} className="p-6 space-y-4">
              <div>
                ...
                <select required value={asignarObraId} onChange={e => setAsignarObraId(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                  <option value="" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">— Elegir —</option>
                  {obras.map(o => <option key={o.id} value={o.id} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">{o.nombre}</option>)}
                </select>
              </div>
              {error && <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">{error}</p>}
              <div className="flex gap-3 pt-1">
