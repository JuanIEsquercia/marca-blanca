...
export default function UnidadForm({ tipologias, unidad, onClose, onSuccess, obraId, constructoraId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-xl max-h-[92vh] overflow-y-auto">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Ubicación</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Piso *</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Unidad *</label>
                <input required value={form.numero} onChange={e => set('numero', e.target.value)}
                  placeholder="A / 01 / 3B"
                  className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Orientación</label>
          ...
          {/* Tipología */}
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Tipología</p>
            {tipologias.length === 0 ? (
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg text-amber-700 dark:text-amber-300 text-sm">
                ...
                {tipologias.map(t => (
                  <label key={t.id}
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-slate-900 dark:text-white text-sm">{t.nombre}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{t.m2_totales} m² totales · {t.descripcion ?? ''}</p>
                    </div>
                    {form.tipologia_id === t.id && (
          ...
          {/* m2 por unidad */}
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Superficie</p>
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                m² propios de esta unidad
                <span className="ml-1 font-normal text-slate-400 dark:text-slate-500">(opcional — sobreescribe el de tipología)</span>
              </label>
              <input type="number" step="0.01" min="0" value={form.m2}
                onChange={e => set('m2', e.target.value)}
                placeholder="48.50"
                className="w-36 px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              {m2Ref && <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{m2Ref}</p>}
            </div>
          </div>

          {/* Precio y condiciones */}
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Precio y condiciones</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="col-span-1">
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Precio USD *</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">% Entrega mín.</label>
                <input type="number" step="0.1" min="0" max="100" value={form.entrega_minima_pct}
                  onChange={e => set('entrega_minima_pct', e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Máx. cuotas</label>
            ...
            {/* Resumen calculado */}
            {form.precio_lista && (
              <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/50 rounded-lg grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-slate-500 dark:text-slate-400">Anticipo mínimo</span>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">
                    {formatCurrency(redondear2(parseFloat(form.precio_lista) * parseFloat(form.entrega_minima_pct) / 100))}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">Precio/m²</span>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">
                    {form.m2 || tipSelected
          ...
          {isEditing && (
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Estado comercial</label>
              <div className={`inline-block px-3 py-1.5 rounded-lg border-2 text-sm font-medium
                ${unidad!.estado_comercial === 'Disponible' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300'
                  : unidad!.estado_comercial === 'Reservado' ? 'border-amber-500 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300'
                  : 'border-slate-500 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              Cancelar
            </button>
