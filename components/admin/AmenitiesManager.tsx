          ...
          return (
            <div key={a.id} className={cn(
              ...
              <div className="flex items-center gap-4 p-4">
                {/* Thumbnail portada */}
                <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                  {portada ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!a.activo && (
                      <span className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">Oculto</span>
                    )}
                  </div>
                  {a.descripcion && <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{a.descripcion}</p>}
                  <p className="text-xs mt-0.5">
                    <span className={cn(
                      'font-medium',
                      atLimit ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400 dark:text-slate-500'
                    )}>
                      {imageCount}/{MAX_IMAGENES} fotos
                    </span>
                    <span className="text-slate-300 dark:text-slate-700 mx-1">·</span>
                    <span className="text-slate-400 dark:text-slate-500">Orden: {a.orden}</span>
                  </p>
                </div>
                ...
                {!readOnly && (
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => handleDelete(a.id, a.nombre)}
                      className="text-xs text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 px-1 transition-colors">
                      ✕
                    </button>
                  </div>
                )}
              ...
              {imageCount > 0 ? (
                  ...
                  {imagenes.map((img, idx) => (
                    <div key={img.id} className="relative shrink-0 group">
                      ...
                      <img
                        ...
                        className="w-36 h-24 object-cover rounded-lg border border-slate-200 dark:border-slate-700"
                      />
                      {!readOnly && (
                        <button
                  ...
                  {!atLimit && !readOnly && (
                    <button
                      ...
                      className={cn(
                        ...
                        'text-slate-400 dark:text-slate-500 text-xs gap-1 transition-colors',
                        draggingOver === a.id
                          ? 'border-indigo-400 dark:border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-500 dark:text-indigo-400'
                          : 'border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 hover:text-indigo-500 dark:hover:text-indigo-400'
                      )}
                    >
                ...
                >
                  <div className={cn(
                    ...
                    draggingOver === a.id
                      ? 'border-indigo-400 dark:border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400'
                      : 'border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-indigo-300 dark:hover:border-indigo-500 hover:text-indigo-500 dark:hover:text-indigo-400'
                  )}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                        d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="number" min="0" value={form.orden}
                  ...
                  className="w-24 px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg text-sm
                             focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Número menor = aparece primero en la landing</p>
              </div>
